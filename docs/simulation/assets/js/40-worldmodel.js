/* =============================================================================
   ...
   Contacts and bridge geometry are derived from SensorObservations. Route
   planning also receives an explicit static navigation chart. Nothing here
...
(function (NS) {
  ...
  function WorldModel(provider) {
    ...
    this.chart = new NS.navigation.Chart(provider.getNavigationChart());
  ...
  /* Evaluate routes against chart banks, estimated contacts and the measured
     bridge opening. The navigation module shares its helm model with the hull. */
  WorldModel.prototype.planAvoidance = function (dt) {
    NS.navigation.plan(this, dt);
  };
